"""Seed missing-price defaults using market estimates (Q1 2026)

Revision ID: 009
Revises: 008
Create Date: 2026-03-17

Adds idempotent EUR/g market-estimate rows for currently missing ingredients,
using supplier_id 'market_estimate_2026_q1'.
"""
...
def upgrade() -> None:
    op.execute(
        ...
        WHERE NOT EXISTS (
            ...
            WHERE ip.ingredient_id = p.ingredient_id
              AND ip.supplier_id = 'market_estimate_2026_q1'
        );
        ...
        SET
            price_per_unit = p.eur_per_gram,
            price_per_gram = p.eur_per_gram,
            currency = 'EUR',
            unit = 'g',
            supplier_name = 'market_estimate',
            article_number = 'default',
            updated_at = NOW()
        FROM price_inputs p
        WHERE ip.ingredient_id = p.ingredient_id
          AND ip.supplier_id = 'market_estimate_2026_q1';
...
def downgrade() -> None:
    op.execute(
        ...
        WHERE supplier_id = 'market_estimate_2026_q1';
        """
    )
