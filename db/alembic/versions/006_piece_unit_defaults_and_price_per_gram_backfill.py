...
def upgrade() -> None:
    op.execute(
        """
        WITH seed(ingredient_id, unit_code, grams_per_unit, label) AS (
            VALUES
                ('f603175a-a720-5841-bd3b-8bb4b58f84f6'::uuid, 'stck', 500,  'Balsamicoessig 1 Stck in g'),
                ('1c0510ee-6447-5f10-b7ca-e52644b9de08'::uuid, 'stck', 1200, 'Blumenkohl 1 Stck in g'),
                ('f662257f-5e60-5eb9-896a-bef62d61d485'::uuid, 'stck', 58,   'Eier 1 Stck in g'),
                ('a8af4063-5b26-5389-9dad-f266ae5e4333'::uuid, 'stck', 350,  'Gurke 1 Stck in g'),
                ('34754ab5-53f2-594c-a99e-a952f92bead0'::uuid, 'stk', 80,    'rote zwiebel 1 Stk in g'),
                ('d690bf27-7a51-5ac2-b801-198058bd835d'::uuid, 'stck', 8,    'Vanillezucker 1 Stck in g'),
                ('3fd1ffda-3064-5c51-8a40-9bffda6fd925'::uuid, 'stck', 750,  'Weisswein trocken 1 Stck in g')
        )
        INSERT INTO public.ingredient_units (ingredient_id, unit_code, grams_per_unit, label)
        SELECT s.ingredient_id, s.unit_code, s.grams_per_unit, s.label
        FROM seed s
