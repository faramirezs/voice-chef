_This project has been created as part of the 42 curriculum by @alramire, @mpeshko, @mekundur, @cbotel, @dmlasko._
...
make us-prod           # production: builds with --no-cache
# OR
make us-prod-fast      # production: builds and runs detached
# OR
...
| CD: VPS deploy | SSH → Hetzner → `make us-prod` → Alembic migrate → health check |
